export interface Monument {
  id: string;
  /** Present when loaded from API (MySQL `city_code`). */
  cityCode?: 'delhi' | 'jaipur';
  name: string;
  nameHindi: string;
  nameFrench: string;
  location: string;
  ...
  category: 'fort' | 'palace' | 'temple' | 'museum' | 'market' | 'garden' | 'monument';
}
...
export const jaipurMonuments: Monument[] = [
  {
    id: 'hawa-mahal',
    name: 'Hawa Mahal',
    nameHindi: 'हवा महल',
...
export function findMonumentByName(query: string, city: string = 'jaipur'): Monument | null {
  const lowerQuery = query.toLowerCase();
  const monuments = city.toLowerCase() === 'delhi' ? delhiMonuments : jaipurMonuments;
...
export function getAllMonuments(city?: string): Monument[] {
  if (city && city.toLowerCase() === 'delhi') {
    return delhiMonuments;
  }
  return jaipurMonuments;
}

export function getNearbyMonuments(lat: number, lng: number, limit: number = 3): Monument[] {
  // Simple distance calculation (Haversine would be better, but for demo purposes)
  const monumentsWithDistance = jaipurMonuments.map(monument => ({
    monument,
    distance: Math.sqrt(
      Math.pow(monument.coordinates.lat - lat, 2) + 
